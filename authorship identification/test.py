...
class FeatureExtractor:
    ...
    def features(self, text):
        d = defaultdict(int)
        text_token = kTOKENIZER.tokenize(text)
        text_len = len(text_token)
        for i in range(text_len):
            d[morphy_stem(text_token[i])] += 1
            d[text_token[i]] += i - 1  # words before
            if i > 0:
                d[text_token[i - 1]] += text_len - i  # words after

        ngram = ngrams(text, 4)
        for i in ngram:
            d[i] += 1

        ngram = ngrams(text_token, 2)
        for i in ngram:
            d[i] += 1
...
if __name__ == "__main__":
    ...
    if testfile is None:
        ...
        '''
        test = DictReader(testfile, delimiter='\t')
        hidden_test = []
        for ii in test:
            if args.subsample < 1.0 and int(ii['id']) % 100 > 100 * args.subsample:
                continue
            feat = fe.features(ii['text'])
            hidden_test.append((feat, ii['cat']))
        # Retrain on all data
        classifier = nltk.classify.NaiveBayesClassifier.train(dev_train + dev_test)
        sys.stderr.write("Accuracy on hidden_dev: %f\n" % nltk.classify.accuracy(classifier, hidden_test))
